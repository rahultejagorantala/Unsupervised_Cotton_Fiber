...
def main():
    ...
    base_dataset = get_train_dataset(p, val_transformations, split='train', to_neighbors_dataset = True) # Dataset w/o augs for knn eval
    base_dataloader = get_val_dataloader(p, base_dataset) 

    # Model
    print(colored('Get model', 'blue'))
    print("p['pretext_model'] :", p['pretext_model'])
    ...
    for epoch in tqdm(range(start_epoch, p['epochs'])):
        ...
        ''' above was val_dataloader before changed to train set base_dataloader'''
        ...
        consistancy_loss.append(scan_stats["scan"][0]["consistency"])
        entropy_loss.append(scan_stats["scan"][0]["entropy"])
        total_loss.append(scan_stats["scan"][0]["total_loss"])

        lowest_loss_head = scan_stats['lowest_loss_head']
        lowest_loss = scan_stats['lowest_loss']
       
        if lowest_loss < best_loss: # changed from validation set to training set below.
            print('New lowest loss on Training set: %.4f -> %.4f' %(best_loss, lowest_loss))
        ...
        else: # changed from validation set to training set below.
            print('No new lowest loss on Training set: %.4f -> %.4f' %(best_loss, lowest_loss))
        ...
        nmi.append(clustering_stats["NMI"])
        ari.append(clustering_stats["ARI"])
        epochs.append(epoch) 
        accuracy_train.append(clustering_stats["ACC"])
    ...
    if len(epochs) > 0:
    # plot_training_curves(hungarian_stats)
        plot_training_curves(None, {"epochs" : epochs , "consistancy_loss" : consistancy_loss, "entropy_loss" : entropy_loss, 
                          "accuracy_train" : accuracy_train, "ari" : ari, "nmi" : nmi, "total_loss" :  total_loss}, None,
                          os.path.dirname(p['scan_checkpoint']))
    ...
    # Predictions have output of shape (batch size, num of classes) and features of shape (batch size, num of features(512))
