...
if os.path.exists(r'C:\Users\AVLguest\work\Unsupervised\Unsupervised-Classification-master\Datasets\cotton\single_frame_1000') == False:
    os.mkdir(r'C:\Users\AVLguest\work\Unsupervised\Unsupervised-Classification-master\Datasets\cotton\single_frame_1000')
    
output_dir = save_dir = r'C:\Users\AVLguest\work\Unsupervised\Unsupervised-Classification-master\Datasets\cotton/single_frame_1000'
input_dir = [ r"C:\Users\AVLguest\3140(K)\3140(K)/", r"C:\Users\AVLguest\3089(DAIJA)\3089(DAIJA)/" ]
    
labels, names, im_size = preprocess(1000, dataset_path)
...
input_train = image_process( os.path.join(save_dir, "Train/"), im_size, images_names_train)
input_test = image_process( os.path.join(save_dir, "Test/"), im_size, images_names_test)


if save:  # set correct path before saving
    np.savetxt(os.path.join(save_dir ,'train_labels.txt'), labels_train, delimiter=',', fmt='%s')
    np.savetxt(os.path.join(save_dir ,'test_labels.txt'), labels_test, delimiter=',', fmt='%s')
    np.savetxt(os.path.join(save_dir ,'train_names.txt'), images_names_train, delimiter=',', fmt='%s')
    np.savetxt(os.path.join(save_dir ,'test_names.txt'), images_names_test, delimiter=',', fmt='%s')
    np.save(os.path.join(save_dir , 'train.npy'), input_train)
    np.save(os.path.join(save_dir ,'test.npy'), input_test)