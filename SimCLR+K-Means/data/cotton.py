...
class COTTON(Dataset):
    ...
    def __init__(self, train_size, val_size, basefolder, root=MyPath.db_root_dir('cotton'), train=True, transform=None, 
        ...
        if self.p['augmentation_strategy'] == "select_image_same_large_video" and self.train:
            large_video_names = []
            videos_with_only_1_image = []
            for image_name in self.names:
                small_video_name = image_name[:image_name.rfind('-')]
                large_video_names.append(small_video_name.split('R')[0])
                # print(image_name[:image_name.rfind('-')].split('R')[0], image_name[:image_name.rfind('-')])
            large_video_names = np.unique(large_video_names)
            # print(len(large_video_names))
            # dd
            # print(video_names, len(video_names))
            frame_count = {}
            remove_indices = []
            for large_video in large_video_names:
                frame_list = []
                for frame in self.names:
                    # print(large_video, frame)
                    small_video_name = frame[:frame.rfind('-')]
                    if small_video_name.split('R')[0] == large_video:
                        frame_list.append(frame)
                        # if '081222DAIJA1-14' == small_video_name.split('R')[0]:
                        #     print(frame_list , "\n")
                if len(frame_list) > 1 :
                    frame_count[large_video] = frame_list 
                else:
                    # print("found large video name with 1 only images.", large_video, frame_list)
                    videos_with_only_1_image.append(large_video)
                    if 'single_frame' in self.p['basefolder']:
                        remove_indices.append(np.where(self.names == frame_list[0])[0])
            print("large videos with 1 only image.", videos_with_only_1_image)
            print("removed from dataset as videos with single frames are not required")
            self.data = np.delete(self.data, remove_indices, axis = 0)
            self.targets = np.delete(self.targets, remove_indices, axis = 0)
            self.names = np.delete(self.names, remove_indices, axis = 0)
            print("New dataset length - ", len(self.data))
            self.vid_img_dict = frame_count
    ...
    def __getitem__(self, index):
        ...
        if self.p['augmentation_strategy'] in ['select_crop', 'select_image_same_video_crop']:
            img = select_crop(index, self, self.p['augmentation_kwargs']['crop_size'], img, self.p['augmentation_kwargs']['num_of_crops'])
