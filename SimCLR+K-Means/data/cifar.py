...
from data.custom_dataset import create_new_dataset, convert_labels
...
class CIFAR10(Dataset):
    ...
    def __init__(self, root=MyPath.db_root_dir('cifar-10'), train=True, transform=None, 
                    download=False, class_indices = None):
        ...
        if class_indices != 'All':
            ...
            new_data, new_targets = create_new_dataset(np.array(self.data), np.array(self.targets), class_indices)
            converted_targets = convert_labels(new_targets)
            # import matplotlib.pyplot as plt
            # for index, i in enumerate(new_data):
            #     plt.imshow(i)
            #     plt.title("index" + str(new_targets[index]) + "  " + str(converted_targets[index]))
            #     plt.show()
            self.data = new_data
            self.targets = converted_targets
        # print(type(self.data), type(self.targets))
        self._load_meta(class_indices)

    def _load_meta(self, class_indices):
        ...
        if class_indices != 'All':
            self.classes = [self.classes[idx] for idx in class_indices]
            self.class_to_idx = {_class: i for i, _class in enumerate(self.classes)}
            print("Classes chosen", self.classes, self.class_to_idx)
