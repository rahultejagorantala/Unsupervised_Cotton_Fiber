...
from utils.evaluate_utils import get_predictions, hungarian_evaluate, plot_training_curves
...
def main():
    ...
    # for tracking training set progress
    evaluating_train_dataset = get_train_dataset(p, val_transforms, split='train', to_augmented_dataset = False)
    evaluating_train_dataloader = get_val_dataloader(p, evaluating_train_dataset)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        print('Train ...')
        loss = selflabel_train(train_dataloader, model, criterion, optimizer, epoch, ema=ema)
        ...
        #evaluate train
        predictions = get_predictions(p, evaluating_train_dataloader, model)
        clustering_stats_train = hungarian_evaluate(0, predictions, compute_confusion_matrix=False) 
        accuracy_train.append(clustering_stats_train["ACC"])
        nmi.append(clustering_stats_train["NMI"])
        ari.append(clustering_stats_train["ARI"])
        epochs.append(epoch)
        
        # hungarian_stats.append(clustering_stats)
        # Checkpoint
        print('Checkpoint ...')
        torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(), 
                    'epoch': epoch + 1}, p['selflabel_checkpoint'])
        torch.save(model.module.state_dict(), p['selflabel_model'])
        # print(epochs, train_loss, accuracy_train, ari, nmi)

    if len(epochs) > 0:
    # plot_training_curves(hungarian_stats)
        plot_training_curves(None, None, {"epochs" : epochs , "loss" : train_loss, 
                          "accuracy_train" : accuracy_train, "ari" : ari, "nmi" : nmi},
                          os.path.dirname(p['selflabel_checkpoint']))
