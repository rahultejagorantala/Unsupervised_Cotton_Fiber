...
def plot_visualize_tsne(input_dir, k_means_labels = None, display = False, name = None, marked_indices = None, perplexity = 30):
   ...
   # Define a custom colormap with distinct colors
    custom_cmap = plt.cm.get_cmap('tab10', len(np.unique(targets)))  # Choose a colormap, such as 'tab10'
...
def kmeans(input_dir, dim, k, display_plot, perplexity,create_tsne = False):
    ...
    accuracy, nmi, ari, reordered_preds = hungarian(labels,targets, k )
    if create_tsne:
        plot_visualize_tsne(input_dir,reordered_preds,display_plot, "Normal K-means,  Acc - " + str(round(accuracy, 2)), indices, perplexity)


def faiss_k_means(input_dir,dim, k,display_plot, perplexity, create_tsne = False, centroids = None):
    import faiss
    import numpy as np
    import centroid_initialization as cent_init
    features = np.load(os.path.join(input_dir, 'features.npy')) # loading cropped data
    ...
    index.add(features)
    # Perform clustering
    if centroids.any() == None:
        plus_centroids, plus_plus_seed = cent_init.plus_plus(features, k , 32)
    else:
        plus_centroids = centroids
    kmeans = faiss.Kmeans(dim, k,seed = 8, nredo = 5)  # 128 is the dimension of your data points
    ...
    accuracy, nmi, ari, reordered_preds = hungarian(clusters.flatten(),targets, k)
    print(accuracy, nmi, ari)
    # print(targets[:100], reordered_preds[:100], clusters.flatten()[:100])
    if create_tsne:
        plot_visualize_tsne(input_dir, reordered_preds.flatten(), display_plot, "faiss K-means, Acc - " + str(round(accuracy, 2)), indices, perplexity)
...
def hungarian(predictions, targets, num_classes):
    ...
    # print(acc, match)
    return acc, nmi, ari, reordered_preds
...
def loop_clustering(input_dir,dim,k, redo, seed_count, faiss_ind=True):
    ...
    avg_accuracy = 0
    avg_nmi = avg_ari = 0
    mean_centroids = mean_cluster_centroids(input_dir)
    ...
    for i in tqdm(range(seed_count)):
        for re in range(redo):
            for j in ['random', 'k-means++']:
                ...
                accuracy, nmi, ari, reordered_preds = hungarian(targets,labels.flatten(), k )
                avg_accuracy+=accuracy
                avg_nmi+=nmi
                avg_ari+=ari
                # hung_accuracy = hungarian_match(targets, labels.flatten())  # Compare with ground truth labels
                print(accuracy)
                distance_bt_centroids(mean_centroids, kmeans.centroids)
                if accuracy > best_accuracy:
                    best_accuracy = accuracy
                    best_labels = labels
                    best_params = {'random_seed' : i,'plus_plus_seed' : plus_plus_seed, 'init' : j, 'Accuracy' :  best_accuracy, 'best_labels' : best_labels, 'nmi': nmi, 'ari': ari, 'centroids' : kmeans.centroids}
    ...
    print("best_accuracy:", best_params['Accuracy'], "best_nmi:", best_params['nmi'] ,"best_ari:", best_params['ari'], "avg_accuracy :", avg_accuracy, "avg_ari :", ari, "avg_nmi :", nmi)
    # # Create an index and add data
    # index = faiss.IndexFlatL2(dim)  # 128 is the dimension of your data points
    # index.add(features)
    # # Perform clustering
...
# basefolder = r"cotton\K-Means\pretext - no aug, 240x240, efficientnet_b1,epochs 10\features"
    return best_params
def loop_clustering_multiple_crops(input_dir,dim,k, redo, seed_count, no_of_sets):
    features = np.load(os.path.join(input_dir, 'features.npy')) # loading cropped data
    targets = np.load(os.path.join(input_dir, 'targets.npy')) # loading cropped data
    print(features.shape, len(targets))
    num_embeddings = len(targets)
    embedding_size = 2048
    set_size = num_embeddings//no_of_sets

    # Reshape the array into sets of 9420 embeddings each
    reshaped_array = features.reshape(-1, set_size, embedding_size)
        ...
        for re in range(redo):
            for j in ['random', 'k-means++']:
                ...
                for i in range(set_size):
                    ...
                    pointer = pointer + i
                    for _ in range(no_of_sets-1):
                        pointer = pointer + set_size
                        labels_for_image.append(labels[pointer])
                    majority = max(set(labels_for_image), key=labels_for_image.count)
                    # print(labels_for_image, majority)
                    final_labels.append(majority)
                final_labels = np.array(final_labels)
                # print(final_labels, final_labels)

                accuracy, nmi, ari, reordered_preds = hungarian(targets[:set_size],final_labels, k )
                avg_accuracy+=accuracy
                avg_nmi+=nmi
                avg_ari+=ari
                # hung_accuracy = hungarian_match(targets, labels.flatten())  # Compare with ground truth labels
...
def mean_cluster_centroids(input_dir):
    features = np.load(os.path.join(input_dir, 'features.npy')) # loading cropped data
    targets = np.load(os.path.join(input_dir, 'targets.npy')) # loading cropped data
    ...
    for target in unique_targets:
        ...
        mean_embedding = np.mean(features[indices], axis=0)
        # print(indices, targets, features[indices].shape, mean_embedding)
        centroids.append(mean_embedding)
    print(np.array(centroids).shape)
    return np.array(centroids)
def distance_bt_centroids(first_centroids, second_centroids):
    from scipy.spatial.distance import euclidean

    # Calculate distances using Euclidean distance
    distance_1_1 = euclidean(first_centroids[0], second_centroids[0])
    distance_2_2 = euclidean(first_centroids[1], second_centroids[1])
    print(distance_1_1, distance_2_2)
...
if multiple_crops:
    ...
    input_dir = input_dir + '-multicrop'
#     best_params = loop_clustering_multiple_crops(input_dir,dim,k, redo, seed_count,no_of_sets = 5)
# else:
#     best_params = loop_clustering(input_dir,dim,k, redo, seed_count, faiss_ind = True)
# print("k means with mean centroids")
centroids = mean_cluster_centroids(input_dir)
# print(best_params['centroids'], centroids)
# distance_bt_centroids(best_params['centroids'], centroids)
faiss_k_means(input_dir, dim, k, False, perplexity, create_tsne = True, centroids =  centroids)
