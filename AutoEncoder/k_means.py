...
def faiss_k_means(input_dir,dim, k,display_plot, perplexity, create_tsne = False):
    ...
    index.add(features)
    # Perform clustering
    plus_centroids, plus_plus_seed = cent_init.plus_plus(features, k , 103)
    kmeans = faiss.Kmeans(dim, k,seed = 47, nredo = 5)  # 128 is the dimension of your data points
...
def loop_clustering(input_dir,dim,k, redo, seed_count, faiss_ind=True):
    ...
    print("avg_accuracy :", avg_accuracy, "avg_ari :", ari, "avg_nmi :", nmi)
    return best_params
    # # Create an index and add data
    # index = faiss.IndexFlatL2(dim)  # 128 is the dimension of your data points
    # index.add(features)
...
# basefolder = r"results\synthetic_cotton\pretext\embeddings"
